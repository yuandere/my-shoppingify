...
export default function ItemCard({ itemData }: { itemData: IItemsData }) {
	...
	return (
		<div className='flex items-center justify-between rounded-xl drop-shadow-[0_2px_6px_rgba(0,0,0,0.1)] w-48 min-h-min max-h-20 bg-white mx-2 my-4 p-4 cursor-pointer transition hover:bg-theme-3 hover:scale-[1.03] hover:drop-shadow-[0_2px_9px_rgba(0,0,0,0.14)]' onClick={(()=>{
			...
			<Tooltip.Root>
				<Tooltip.Trigger asChild>
					<span className='material-icons text-ui cursor-pointer hover:text-ui-dark'>add</span>
				</Tooltip.Trigger>
				<Tooltip.Portal>
